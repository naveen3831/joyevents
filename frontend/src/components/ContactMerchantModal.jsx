...
import { sanitizeEmailInput, sanitizeNameInput, sanitizeMessageInput, validateEmail, validateName, validateMessage, NAME_MAX_LENGTH, EMAIL_MAX_LENGTH, MESSAGE_MAX_LENGTH, NAME_HINT, EMAIL_HINT, MESSAGE_HINT, } from "@/lib/validation";
const ContactMerchantModal = ({ itemTitle, eventId, serviceId, merchantId, onClose }) => {
    const { user } = useAuth();
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        {sent ? (<div className="text-center py-6 space-y-3">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <Input value={name} maxLength={NAME_MAX_LENGTH} onChange={e => setName(sanitizeNameInput(e.target.value))} placeholder="Your name" className="mt-1" required/>
              ...
              <div>
                ...
                <textarea value={message} maxLength={MESSAGE_MAX_LENGTH} onChange={e => setMessage(sanitizeMessageInput(e.target.value))} placeholder="Ask anything about this event or service..." rows={4} required className="mt-1 w-full rounded-lg border border-border bg-secondary px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary resize-none"/>
