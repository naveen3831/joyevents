...
const NavLinks = ({ onClose }) => {
    ...
    const visibleLinks = isMerchantActive
        ? links
        : links.filter(l => l.to === "/merchant-dashboard" ||
    ...
    return (<nav className="space-y-1 p-3">
      {visibleLinks.map(({ to, label, icon: Icon, ...rest }) => {
            ...
            const isHighlight = rest.highlight;
            return (<Link key={to} to={to} onClick={onClose} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all ${isActive ? "bg-gradient-primary text-primary-foreground"
