...
const NavLinks = ({ onClose }) => {
    ...
    useEffect(() => {
        const active = links.find(l => l.submenu?.some((s) => s.to === location.pathname));
        if (active)
            setExpandedSection(active.label);
    }, [location.pathname]);
    return (<nav className="space-y-1 p-3">
      {links.map((link) => {
            if (link.submenu) {
                const isExpanded = expandedSection === link.label;
                const isActive = link.submenu.some((s) => s.to === location.pathname);
                return (<div key={link.label}>
              <button onClick={() => setExpandedSection(isExpanded ? null : link.label)} className={`w-full flex items-center justify-between gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all ${isActive || isExpanded ? "bg-secondary text-foreground border border-border" : "text-muted-foreground hover:bg-secondary hover:text-foreground"}`}>
                <div className="flex items-center gap-3"><link.icon className="h-4 w-4"/>{link.label}</div>
                <ChevronDown className={`h-4 w-4 transition-transform ${isExpanded ? "rotate-180" : ""}`}/>
              </button>
              <AnimatePresence>
                {isExpanded && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden ml-3 mt-1 border-l border-border pl-3">
                    {link.submenu.map((sub) => {
                            const isSubActive = location.pathname === sub.to;
                            return (<Link key={sub.to} to={sub.to} onClick={onClose} className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-all ${isSubActive ? "bg-gradient-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary hover:text-foreground"}`}>
            ...
            return (<Link key={link.to} to={link.to} onClick={onClose} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all ${isActive ? "bg-gradient-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary hover:text-foreground"}`}>
