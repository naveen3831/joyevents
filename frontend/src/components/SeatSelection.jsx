...
const TIER_EMOJI = { diamond: "💎", gold: "🥇", silver: "🥈" };
// Generate seat labels: A1, A2 … B1 …
function generateSeats(available, sold, prefix, bookedSeats = []) {
...
// ─── Component ────────────────────────────────────────────────────────────────
const SeatSelection = ({ eventType, tickets = [], maxAttendees = 0, attendeesCount = 0, price = 0, bookedSeats = [], onConfirm, onClose, inline = false }) => {
    // ── Ticketed state ──────────────────────────────────────────────────────────
    const [selectedSeats, setSelectedSeats] = useState({});
    // ── FullService state ───────────────────────────────────────────────────────
    const [fsSelectedSeats, setFsSelectedSeats] = useState([]);
    // ── Sorted tickets: diamond → gold → silver → others ───────────────────────
    const sortedTickets = useMemo(() => {
        const known = TIER_ORDER.map(t => tickets.find(tk => tk.type === t)).filter(Boolean);
    ...
    const totalSelected = useMemo(() => Object.values(selectedSeats).reduce((s, arr) => s + arr.length, 0), [selectedSeats]);
    // ── Confirm ─────────────────────────────────────────────────────────────────
    const handleConfirm = () => {
    ...
    return (<div className="flex flex-col h-full">
      ...
      {!inline && (<div className="flex items-center justify-between mb-4 pb-3 border-b border-border">
          <div>
      ...
      {eventType === "ticketed" && (<div className="flex-1 overflow-y-auto space-y-6 pr-1">
          {sortedTickets.map(ticket => {
                ...
                return (<div key={ticket.type} className={`rounded-xl border ${colors.border} ${colors.bg} p-4`}>
                {/* Tier header */}
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="text-right">
                    ...
                    {tierSelected.length > 0 && (<p className={`text-xs font-semibold ${colors.text}`}>{tierSelected.length} selected</p>)}
                  </div>
                </div>

                {avail === 0 ? (<div className="text-center py-3 text-sm text-red-400 font-semibold">SOLD OUT</div>) : (<div className="flex flex-wrap gap-1.5">
                    {seats.map(seat => {
                            const isSelected = tierSelected.includes(seat.id);
                            return (<button key={seat.id} disabled={seat.isSold} onClick={() => toggleSeat(ticket.type, seat.id)} title={seat.id.split("-")[1]} className={`w-8 h-8 rounded text-[10px] font-bold border transition-all duration-150
                            ${seat.isSold
      ...
      {eventType === "fullService" && (<div className="flex-1 overflow-y-auto pr-1">
          <div className="rounded-xl border border-border bg-secondary/20 p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-right">
                ...
                {fsSelectedSeats.length > 0 && (<p className="text-xs font-semibold text-primary">{fsSelectedSeats.length} selected</p>)}
              </div>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {fsSeats.map(seat => {
                const isSelected = fsSelectedSeats.includes(seat.id);
                return (<button key={seat.id} disabled={seat.isSold} onClick={() => toggleFsSeat(seat.id)} title={seat.id.split("-")[1]} className={`w-8 h-8 rounded text-[10px] font-bold border transition-all duration-150
                      ${seat.isSold
                        ? "bg-red-500/20 border-red-500/30 text-red-400/50 cursor-not-allowed"
                        : isSelected
      ...
      {!inline && (<div className="mt-4 pt-4 border-t border-border space-y-3">
          {eventType === "ticketed" && totalSelected > 0 && (<div className="space-y-1 text-sm">
              {sortedTickets.map(t => {
                    const count = selectedSeats[t.type]?.length || 0;
                    if (!count)
                        return null;
                    return (<div key={t.type} className="flex justify-between text-muted-foreground capitalize">
          ...
          {eventType === "fullService" && fsSelectedSeats.length > 0 && (<div className="flex justify-between font-bold text-base">
          ...
          <Button className="w-full bg-gradient-primary text-primary-foreground hover:opacity-90" size="lg" disabled={eventType === "ticketed" ? totalSelected === 0 : fsSelectedSeats.length === 0} onClick={handleConfirm}>
