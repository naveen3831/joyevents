...
const SimplePayment = ({ amount, bookingId, bookingData, onSuccess, onError, onClose, initUseWallet }) => {
    ...
    return (<div className="space-y-3">
      ...
      {remainingAmount > 0 ? (<>
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setSelectedMethod("card")} className={`flex items-center justify-center gap-2 p-2 rounded-lg border-2 transition-all ${selectedMethod === "card"
                ? "border-primary bg-primary/10"
                : "border-border hover:border-primary/50"}`}>
                <CreditCard className="h-4 w-4"/>
                <span className="font-medium text-xs">Card</span>
              </button>
              <button type="button" onClick={() => setSelectedMethod("upi")} className={`flex items-center justify-center gap-2 p-2 rounded-lg border-2 transition-all ${selectedMethod === "upi"
                ? "border-primary bg-primary/10"
                : "border-border hover:border-primary/50"}`}>
          ...
          <div className="space-y-2">
            {selectedMethod === "card" ? (<>
                <div>
                  ...
                  <Input id="cardNumber" placeholder="1234 5678 9012 3456" value={cardNumber} onChange={(e) => setCardNumber(formatCardNumber(e.target.value))} maxLength={19} className="h-9 text-sm mt-1"/>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <Label htmlFor="expiryDate" className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Expiry Date</Label>
                    <Input id="expiryDate" placeholder="MM/YY" value={expiryDate} onChange={(e) => setExpiryDate(formatExpiryDate(e.target.value))} maxLength={5} className="h-9 text-sm mt-1"/>
                  </div>
                  <div>
                    <Label htmlFor="cvv" className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">CVV</Label>
                    <Input id="cvv" placeholder="123" value={cvv} type="password" onChange={(e) => setCvv(e.target.value.replace(/[^0-9]/g, ""))} maxLength={3} className="h-9 text-sm mt-1"/>
                  </div>
                </div>
                <div>
                  ...
                  <Input id="cardholderName" placeholder="John Doe" value={cardholderName} onChange={(e) => setCardholderName(e.target.value)} className="h-9 text-sm mt-1"/>
                </div>
              </>) : (<div>
                <Label htmlFor="upiId" className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">UPI ID</Label>
                <Input id="upiId" placeholder="yourname@paytm" value={upiId} onChange={(e) => setUpiId(e.target.value)} className="h-9 text-sm mt-1"/>
              </div>)}
          </div>
      ...
      {/* Payment Button */}
      <Button onClick={handlePayment} disabled={loading} className="w-full bg-gradient-primary hover:opacity-90 text-white font-bold" size="sm">
        {loading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Processing...</>) : remainingAmount === 0 ? (<>Pay {formatCurrency(walletApplied)} with Wallet</>) : (<>Pay Remaining {formatCurrency(remainingAmount)} {bookingData.paymentType === "remaining" ? "& Complete" : "& Confirm"}</>)}
      </Button>
