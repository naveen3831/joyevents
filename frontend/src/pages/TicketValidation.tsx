              ...
              <div className="rounded-xl border border-border bg-card p-6">
                ...
                {historyLoading ? (
                  ...
                  <div className="space-y-2">
                    {(showAllValidations ? searchHistory : searchHistory.slice(0, 5)).map((item, idx) => (
                      <div key={idx} className="p-3 rounded bg-secondary/50 text-xs space-y-1 border border-border/50">
                        ...
                        {item.customerName && (
                          <p className="text-[11px] text-foreground/80 font-medium">Customer: {item.customerName}</p>
                        )}
                        {item.eventTitle && (
                          <p className="text-[10px] text-muted-foreground truncate">Event: {item.eventTitle}</p>
                        )}
