...
export default function QRCodeGenerator() {
    ...
    const handleToggleActive = async (id, type, currentActive) => {
        if (!token)
            return;
        try {
            const apiUrl = API_URL;
                    ...
                    <CardContent className="flex-1 space-y-2 sm:space-y-4 p-3 sm:p-6">
                      {event.qrCodeCustomUrl ? (<>
                          <div className={`flex flex-col items-center justify-center p-4 bg-secondary rounded-lg relative transition-opacity ${!event.qrCodeActive ? 'opacity-40 grayscale' : ''}`}>
                            <div ref={(el) => { if (el)
                    qrRefs.current[`event-${event._id}`] = el; }} className="bg-white p-3 rounded-md shadow-sm flex items-center justify-center">
                              <QRCodeSVG value={event.qrCodeCustomUrl} size={120} level="H" includeMargin={true} fgColor="#000000" bgColor="#ffffff"/>
                            </div>
                            {!event.qrCodeActive && (<div className="absolute inset-0 flex items-center justify-center bg-background/20 rounded-lg">
                          ...
                          <div className="flex items-center justify-between p-2 bg-secondary/50 rounded-lg">
                            <div className="flex items-center gap-2">
                              {event.qrCodeActive ? (<Power className="h-4 w-4 text-green-500"/>) : (<PowerOff className="h-4 w-4 text-muted-foreground"/>)}
                              <span className="text-xs font-medium">
                                {event.qrCodeActive ? 'Active' : 'Inactive'}
                              </span>
                            </div>
                            <Switch checked={event.qrCodeActive} onCheckedChange={() => handleToggleActive(event._id, 'event', !!event.qrCodeActive)}/>
                          </div>

                          <div className="space-y-2">
                            <Button variant="outline" size="sm" className="w-full" disabled={!event.qrCodeActive} onClick={() => downloadQRCode(`event-${event._id}`, `event-${event.title}`)}>
                              <Download className="h-4 w-4 mr-2"/>
                              Download
                            </Button>
                            <div className="flex gap-2">
                              <Button variant="outline" size="sm" className="flex-1" onClick={() => openUpdateDialog(event._id, 'event', event.qrCodeCustomUrl)}>
                                <Edit2 className="h-4 w-4 mr-2"/>
                                Edit
                              </Button>
                              <Button variant="outline" size="sm" className="flex-1 text-destructive hover:bg-destructive/10" onClick={() => handleDeleteQR(event._id, 'event')}>
                                <Trash2 className="h-4 w-4 mr-2"/>
                                Delete
                              </Button>
                            </div>
                            <Button variant="secondary" size="sm" className="w-full" onClick={() => copyToClipboard(event.qrCodeCustomUrl || "", `event-${event._id}`)}>
                          ...
                          <Button size="sm" onClick={() => openUpdateDialog(event._id, 'event')}>
                            Create QR Code
                          </Button>
                        </div>)}
                    </CardContent>
                    ...
                    <CardContent className="flex-1 space-y-2 sm:space-y-4 p-3 sm:p-6">
                      {service.qrCodeCustomUrl ? (<>
                          <div className={`flex flex-col items-center justify-center p-4 bg-secondary rounded-lg relative transition-opacity ${!service.qrCodeActive ? 'opacity-40 grayscale' : ''}`}>
                            <div ref={(el) => { if (el)
                    qrRefs.current[`service-${service._id}`] = el; }} className="bg-white p-3 rounded-md shadow-sm flex items-center justify-center">
                              <QRCodeSVG value={service.qrCodeCustomUrl} size={120} level="H" includeMargin={true} fgColor="#000000" bgColor="#ffffff"/>
                            </div>
                            {!service.qrCodeActive && (<div className="absolute inset-0 flex items-center justify-center bg-background/20 rounded-lg">
                                <span className="bg-destructive/10 text-destructive px-3 py-1 rounded-full text-xs font-bold border border-destructive/20 backdrop-blur-sm">
                                  INACTIVE
                                </span>
                              </div>)}
                          ...
                          <div className="flex items-center justify-between p-2 bg-secondary/50 rounded-lg">
                            <div className="flex items-center gap-2">
                              {service.qrCodeActive ? (<Power className="h-4 w-4 text-green-500"/>) : (<PowerOff className="h-4 w-4 text-muted-foreground"/>)}
                              <span className="text-xs font-medium">
                                {service.qrCodeActive ? 'Active' : 'Inactive'}
                              </span>
                            </div>
                            <Switch checked={service.qrCodeActive} onCheckedChange={() => handleToggleActive(service._id, 'service', !!service.qrCodeActive)}/>
                          </div>

                          <div className="space-y-2">
                            <Button variant="outline" size="sm" className="w-full" disabled={!service.qrCodeActive} onClick={() => downloadQRCode(`service-${service._id}`, `service-${service.name}`)}>
                              <Download className="h-4 w-4 mr-2"/>
                              Download
                            </Button>
                            <div className="flex gap-2">
                              <Button variant="outline" size="sm" className="flex-1" onClick={() => openUpdateDialog(service._id, 'service', service.qrCodeCustomUrl)}>
                                <Edit2 className="h-4 w-4 mr-2"/>
                                Edit
                              </Button>
                              <Button variant="outline" size="sm" className="flex-1 text-destructive hover:bg-destructive/10" onClick={() => handleDeleteQR(service._id, 'service')}>
                                <Trash2 className="h-4 w-4 mr-2"/>
                                Delete
                              </Button>
                            </div>
                            <Button variant="secondary" size="sm" className="w-full" onClick={() => copyToClipboard(service.qrCodeCustomUrl || "", `service-${service._id}`)}>
                          ...
                          <Button size="sm" onClick={() => openUpdateDialog(service._id, 'service')}>
                            Create QR Code
                          </Button>
                        </div>)}
                    </CardContent>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input id="url" value={customUrl} onChange={(e) => setCustomUrl(e.target.value)} placeholder="https://example.com"/>
