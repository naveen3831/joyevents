...
const Index = () => {
    ...
    return (<Layout>
      ...
      <section className="py-12 sm:py-20 bg-red-500/5">
        <div className="container mx-auto">
          ...
          {loading ? (<div className="flex items-center justify-center py-16 text-muted-foreground gap-2">
            ...
            </div>) : liveEvents.length === 0 ? (<div className="py-20 text-center text-muted-foreground border border-border rounded-xl bg-card">
              ...
              {liveEvents.map((event, idx) => (<motion.div key={event._id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.07 }}>
      ...
      {/* Promo Codes Section */}
      {!promoLoading && promoCodes.length > 0 && (<section className="py-20 bg-gradient-to-r from-amber-500/10 via-orange-500/10 to-red-500/10">
          <div className="container mx-auto">
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {(showAllPromos ? promoCodes : promoCodes.slice(0, 3)).map((promo, idx) => (<motion.div key={promo._id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }} className="rounded-xl border border-amber-200/50 bg-gradient-to-br from-amber-50/50 to-orange-50/50 dark:from-amber-950/20 dark:to-orange-950/20 p-5 hover-lift">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-2">
                        ...
                        {promo.minBookingAmount > 0 && (<span className="text-[10px] font-semibold rounded-full bg-amber-500/15 text-amber-600 px-2 py-0.5">
                            Min {formatCurrency(promo.minBookingAmount)}
                          </span>)}
                      </div>
                      {promo.minBookingAmount > 0 && (<p className="text-[11px] text-amber-700/90 dark:text-amber-300 mt-1">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-amber-200/30">
                    ...
                    <div className="flex items-center gap-3">
                      {promo.merchant?.name && (<span className="text-[11px] text-muted-foreground">{promo.merchant.name}</span>)}
            ...
            {promoCodes.length > 3 && (<div className="mt-8 text-center">
                <button onClick={() => setShowAllPromos(v => !v)} className="inline-flex items-center gap-2 rounded-full border border-amber-500/40 bg-amber-500/10 px-6 py-2.5 text-sm font-semibold text-amber-600 hover:bg-amber-500/20 transition-colors">
                  {showAllPromos ? (<>Show Less</>) : (<>View All {promoCodes.length} Offers <ArrowRight className="h-4 w-4"/></>)}
                </button>
              </div>)}
          </div>
        </section>)}
        ...
        <div className="container mx-auto">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {loading ? (<div className="flex items-center justify-center py-16 text-muted-foreground gap-2 col-span-full">
              ...
              </div>) : liveEvents.length > 0 ? (liveEvents.slice(0, 4).map((event, idx) => (<motion.div key={event._id + "-featured"} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.07 }}>
                  <EventCard event={event} index={idx} onBookNow={handleBookEvent} onViewDetails={(e) => navigate(`/events/${e._id}`)} onImageClick={(imageIdx) => handleImageClick(event, imageIdx)}/>
                </motion.div>))) : (<div className="py-12 text-center text-muted-foreground col-span-full">
                No featured events at the moment.
              </div>)}
          </div>

          <div className="mt-8 flex justify-center">
            <Button onClick={() => navigate("/events")} variant="outline" className="px-6 border-primary/20 hover:border-primary/50 text-foreground gap-2 font-semibold">
        ...
        <div className="container mx-auto">
          ...
          {svcLoading ? (<div className="flex items-center gap-2 text-muted-foreground py-6">
              ...
              <div className="mt-8 grid grid-cols-2 gap-3 md:gap-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {filteredServices.slice(0, 4).map((svc, i) => (<motion.div key={svc._id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }} className="group rounded-2xl border border-border bg-card overflow-hidden flex flex-col hover:border-primary/50 transition-colors">
                    {/* Image */}
                    ...
                    <div className="p-3 sm:p-5 flex flex-col flex-1">
                      ...
                      {/* Highlights */}
                      {svc.highlights?.length > 0 && (<ul className="space-y-1 mb-4 flex-1">
                          {svc.highlights.slice(0, 2).map((h, hi) => (<li key={hi} className="flex items-center gap-1.5 text-[10px] sm:text-xs text-muted-foreground">
                      ...
                      <div className="space-y-2 mt-auto">
                        <Button variant="outline" className="w-full rounded-lg py-1.5 sm:py-2 text-[11px] sm:text-sm font-semibold border-border hover:bg-secondary text-foreground" onClick={() => navigate(`/services/${svc._id}`)}>
                          View Details
                        </Button>
      ...
      {reviews.length > 0 && (<section className="py-12 sm:py-20">
          <div className="container mx-auto">
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="mb-8 sm:mb-12">
              <div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {reviews.map((review, i) => (<motion.div key={review._id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }} className="rounded-2xl border border-border bg-card p-5 flex flex-col gap-3 hover:border-primary/30 transition-colors">
                  {/* Stars */}
                  <div className="flex items-center justify-between">
                    <div className="flex gap-0.5">
                      {[1, 2, 3, 4, 5].map(s => (<Star key={s} className={`h-4 w-4 ${s <= review.score ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground/20"}`}/>))}
                    </div>
                  ...
                  <div className="flex items-center gap-2 pt-2 border-t border-border">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {review.ratedAt && (<p className="text-[10px] text-muted-foreground">
            ...
            {reviews.length >= 6 && (<div className="mt-8 text-center">
                <Link to="/reviews">
