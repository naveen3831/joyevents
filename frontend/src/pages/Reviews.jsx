...
const StarRating = ({ score }) => (<div className="flex gap-0.5">
    {[1, 2, 3, 4, 5].map(s => (<Star key={s} className={`h-4 w-4 ${s <= score ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground/30"}`}/>))}
  </div>);
const Reviews = () => {
    ...
    return (<Layout>
      ...
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="container mx-auto relative">
          ...
          {/* Stats */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
            {[
        ...
        ].map((stat, i) => (<div key={i} className="rounded-2xl border border-border bg-card p-4 text-center">
      ...
      <section className="py-6 border-y border-border bg-secondary/20">
        <div className="container mx-auto flex flex-wrap items-center gap-3">
          ...
          {["all", "event", "service"].map(f => (<button key={f} onClick={() => setFilter(f)} className={`rounded-full px-4 py-1.5 text-sm font-medium border transition-colors capitalize ${filter === f ? "bg-primary text-primary-foreground border-primary" : "border-border bg-card hover:border-primary/50"}`}>
              {f === "all" ? "All Reviews" : f === "event" ? `Events (${eventReviews})` : `Services (${serviceReviews})`}
            </button>))}
          <div className="ml-auto flex items-center gap-2">
            <span className="text-sm text-muted-foreground">Rating:</span>
            <div className="flex gap-1">
              {[0, 3, 4, 5].map(s => (<button key={s} onClick={() => setSelectedScore(s)} className={`rounded-full px-3 py-1 text-xs font-medium border transition-colors ${selectedScore === s ? "bg-primary text-primary-foreground border-primary" : "border-border bg-card hover:border-primary/50"}`}>
                  {s === 0 ? "All" : `${s}★`}
                </button>))}
            </div>
          </div>
      ...
      <section className="py-16">
        <div className="container mx-auto">
          {loading ? (<div className="flex items-center justify-center py-20 gap-3 text-muted-foreground">
              <Loader2 className="h-6 w-6 animate-spin"/> Loading reviews…
            </div>) : filtered.length === 0 ? (<div className="text-center py-20 text-muted-foreground">
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-6 md:grid-cols-2 lg:grid-cols-3">
                {(showAll ? filtered : filtered.slice(0, 6)).map((review, i) => (<motion.div key={review._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="rounded-2xl border border-border bg-card overflow-hidden hover:border-primary/30 transition-colors group">
                    {/* Service/Event image */}
                    {imgSrc(review.image) && (<div className="relative h-36 overflow-hidden bg-secondary">
                    ...
                    <div className="p-5">
                      ...
                      {/* Comment */}
                      {review.comment ? (<div className="relative">
                        ...
                        </div>) : (<p className="text-xs text-muted-foreground italic">No written review</p>)}
                      ...
                      <div className="mt-4 pt-3 border-t border-border flex items-center justify-between">
                        ...
                        {review.ratedAt && (<div className="flex items-center gap-1 text-xs text-muted-foreground">
              ...
              {filtered.length > 6 && (<div className="mt-8 flex justify-center">
                  <Button onClick={() => setShowAll(!showAll)} variant="outline" className="px-6 border-primary/20 hover:border-primary/50 text-foreground font-semibold">
