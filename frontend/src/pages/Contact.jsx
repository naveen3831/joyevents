      ...
      <section className="container mx-auto py-20">
        <div className="grid gap-12 lg:grid-cols-2">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            const cardContent = (<div className="flex items-start gap-2 sm:gap-4">
            ...
            if (item.href) {
                return (<a key={item.title} href={item.href} target={item.href.startsWith("http") ? "_blank" : undefined} rel={item.href.startsWith("http") ? "noreferrer noopener" : undefined} className="rounded-xl border border-border bg-card p-3 sm:p-4 transition-colors hover:border-primary hover:bg-primary/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">
                      {cardContent}
                    </a>);
            }
            return (<div key={item.title} className="rounded-xl border border-border bg-card p-3 sm:p-4">
                    {cardContent}
                  </div>);
        })}
            </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="relative mt-8 overflow-hidden rounded-3xl shadow-2xl">
              <img src={settings.contactImage || "https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=900&q=80"} alt="Our team ready to assist" className="h-[320px] w-full object-cover"/>
              ...
              <form onSubmit={handleSubmit} className="grid gap-4">
                <div className="grid grid-cols-2 gap-3 sm:gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <Input placeholder="John Doe" maxLength={NAME_MAX_LENGTH} value={form.name} onChange={(e) => setForm({ ...form, name: sanitizeNameInput(e.target.value) })} className="bg-secondary" required/>
                ...
                <div>
                  ...
                  <Input placeholder="Wedding enquiry" maxLength={SUBJECT_MAX_LENGTH} value={form.subject} onChange={(e) => setForm({ ...form, subject: sanitizeSubjectInput(e.target.value) })} className="bg-secondary"/>
                  <p className="mt-1 text-xs text-muted-foreground">{SUBJECT_HINT}</p>
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-muted-foreground">Message *</label>
                  <Textarea placeholder="Tell us about your event..." rows={6} maxLength={MESSAGE_MAX_LENGTH} value={form.message} onChange={(e) => setForm({ ...form, message: sanitizeMessageInput(e.target.value) })} className="bg-secondary resize-none" required/>
