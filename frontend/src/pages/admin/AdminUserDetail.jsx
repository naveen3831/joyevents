...
const CopyEmailButton = ({ email }) => {
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className="h-6 w-6 rounded flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-colors cursor-pointer shrink-0"
  ...
  return (
    <AdminLayout>
      <div className="w-full min-w-0 space-y-4 font-sans">
        {/* Mobile Detail Title Row (< 768px) */}
        <div className="flex md:hidden items-center gap-3 py-2.5 px-3 border-b border-border/70 bg-card -mx-4 -mt-5 mb-3 sticky top-0 z-30 shadow-2xs">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="h-10 w-10 rounded-xl bg-muted/60 hover:bg-muted border border-border/60 flex items-center justify-center text-foreground transition-colors shrink-0 cursor-pointer"
            aria-label="Go Back"
          >
            <ArrowLeft className="h-5 w-5" />
          </button>
          <h2 className="text-base font-bold text-foreground truncate">
            {user?.role === "merchant" ? "Merchant Details" : "User Details"}
          </h2>
        </div>

        {/* Desktop Back Navigation (>= 768px) */}
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div className="rounded-2xl border border-border/70 bg-card p-4 sm:p-6 shadow-xs space-y-4">
              {/* Profile Header */}
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border/60 pb-4">
                <div className="flex items-start sm:items-center gap-3.5 min-w-0 w-full sm:w-auto">
                  <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center text-primary text-lg sm:text-xl font-bold shrink-0">
                    {user.name ? user.name.slice(0, 2).toUpperCase() : "U"}
                  </div>
                  <div className="min-w-0 flex-1 space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h1 className="font-bold text-base sm:text-lg text-foreground leading-tight truncate">{user.name}</h1>
                ...
                {/* Primary Actions Bar */}
                <div className="flex items-center gap-2 w-full sm:w-auto justify-end shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-border/40 flex-wrap">
                  ...
                  {user.role === "merchant" && user.merchantStatus !== "active" && user.merchantStatus !== "paid" && (
                    <Button
                      ...
                      className="h-9 px-3.5 text-xs font-semibold text-amber-700 bg-amber-50 hover:bg-amber-100 dark:bg-amber-950/40 dark:text-amber-400 border-amber-200 dark:border-amber-800/60 rounded-xl"
                  ...
                  {user.role === "merchant" && user.merchantStatus === "paid" && (
                    <Button
                      ...
                      className="h-9 px-3.5 text-xs font-semibold text-purple-700 bg-purple-50 hover:bg-purple-100 dark:bg-purple-950/40 dark:text-purple-400 border-purple-200 dark:border-purple-800/60 rounded-xl"
                  ...
                  {user.role === "merchant" && (
                    <Button
                      ...
                      size="sm"
                      onClick={() => navigate(`/admin-dashboard/users/${user._id}/limits`)}
                      className="h-9 px-3.5 text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/40 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800/60 rounded-xl"
                  ...
                  <div className="space-y-2 text-xs">
                    ...
                    <div className="flex justify-between items-center py-1 border-b border-border/40">
                      ...
                      <div className="flex items-center gap-1">
                        <span className="font-semibold text-foreground truncate max-w-[200px]" title={user.email}>{user.email}</span>
                        <CopyEmailButton email={user.email} />
                      </div>
                    </div>
                    <div className="flex justify-between py-1 border-b border-border/40">
