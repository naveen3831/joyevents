...
import { Wallet, TrendingUp, Users, CreditCard, CheckCircle, Clock, ArrowRight, XCircle, Loader2, AlertCircle } from "lucide-react";
...
const AdminPayouts = () => {
    ...
    const handleProcessPayout = async () => {
        ...
        try {
            ...
            const merchantBookingIds = bookings
                ...
                .map(b => b._id);
            await apiProcessMerchantPayout(selectedMerchant._id, Math.round(payout.netPayout * 100) / 100, merchantBookingIds, token);
    ...
    const openPayoutDialog = (merchant) => {
    ...
    return (<AdminLayout>
      ...
      <Dialog open={payoutDialogOpen} onOpenChange={setPayoutDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setPayoutDialogOpen(false)} disabled={processing}>
              Cancel
            </Button>
            <Button onClick={handleProcessPayout} disabled={processing} className="bg-primary hover:bg-primary/90">
              {processing ? (<>
