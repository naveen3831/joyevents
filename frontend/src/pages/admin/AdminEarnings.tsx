...
import { 
  DollarSign, TrendingUp, Users, CreditCard, Clock, 
  ArrowRight, Loader2, AlertCircle, Calculator, Wallet, ShieldCheck, Tag
} from "lucide-react";
...
const AdminEarnings = () => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<"summary" | "merchants" | "commissions">("summary");
  ...
  const onboardingMerchants = allUsers.filter(
    (u: any) => u.role === "merchant" && (u.merchantStatus === "active" || u.merchantStatus === "paid")
  );
  const totalOnboardingEarnings = onboardingMerchants.reduce(
    (sum: number, u: any) => sum + (u.quotationAmount || 0), 0
  );

  // 2. Calculate Limit Upgrade Earnings (Tickets that are paid or approved)
  const paidUpgradeTickets = tickets.filter(
    (t: any) => t.status === "paid" || t.status === "approved"
  );
  const totalUpgradeEarnings = paidUpgradeTickets.reduce(
    (sum: number, t: any) => sum + (t.quotationAmount || 0), 0
  );
  ...
  const totalCommissionEarnings = paidBookings.reduce(
    (sum: number, b: any) => sum + (getPaidAmount(b) * commissionRate), 0
  );
      ...
      <section className="py-6 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <Button 
            ...
            disabled={loading} 
            variant="outline"
            className="border-[#A68C73]/40 text-[#A68C73] hover:bg-[#A68C73]/5"
          >
          ...
          <>
            ...
            <div className="flex border-b border-border mt-8 flex-wrap">
              <button
                ...
                className={`font-semibold text-sm pb-2.5 px-4 border-b-2 transition-all ${
            ...
            {activeSubTab === "summary" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid gap-6 md:grid-cols-2 mt-4">
                  ...
                  <CardContent className="space-y-5">
                    <div className="space-y-2">
                      ...
                      <div className="w-full h-3 rounded-full bg-secondary overflow-hidden">
                        <div 
                          className="h-full bg-[#A68C73] transition-all"
                          style={{ width: `${(totalMerchantEarnings / (totalAdminEarnings || 1)) * 100}%` }}
                        />
                      </div>
                    ...
                    <div className="space-y-2 pt-2">
                      ...
                      <div className="w-full h-3 rounded-full bg-secondary overflow-hidden">
                        <div 
                          ...
                          style={{ width: `${(totalCommissionEarnings / (totalAdminEarnings || 1)) * 100}%` }}
                        />
                      </div>
                ...
                <Card className="bg-card border-border">
                  ...
                  <CardContent className="p-0 overflow-hidden overflow-x-auto">
                    <Table>
                      ...
                      <TableBody>
                        {/* Onboarding Logs */}
                        {onboardingMerchants.map((u: any) => (
                          <TableRow key={`onb-${u._id}`} className="border-border hover:bg-secondary/15 transition-colors">
                        ...
                        {/* Upgrade Ticket Logs */}
                        {paidUpgradeTickets.map((t: any) => (
                          <TableRow key={`tkt-${t._id}`} className="border-border hover:bg-secondary/15 transition-colors">
                            <TableCell className="align-middle font-medium py-3">
                              <div className="flex flex-col">
                        ...
                        {(onboardingMerchants.length === 0 && paidUpgradeTickets.length === 0) && (
                          <TableRow>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {paidBookings.map((b: any) => {
                          ...
                          return (
                            <TableRow key={b._id} className="border-border hover:bg-secondary/15 transition-colors">
