...
import { Shield, Bell, Globe, Check } from "lucide-react";
...
import { apiGetPlatformSettings, apiSavePlatformSettings } from "@/lib/api";
...
function validatePlatformName(value) {
    ...
    if (trimmed.length < 2)
        return "Platform name must be at least 2 characters";
      ...
      <section className="mt-8 space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="rounded-xl border border-border bg-card p-3 sm:p-6">
            ...
            { key: "merchantVerificationAlerts", label: "Merchant verification alerts" },
                  ...
                  <Switch checked={adminPreferences[item.key]} onCheckedChange={(checked) => updatePreference(item.key, checked)} disabled={loading || saving}/>
                </div>))}
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="rounded-xl border border-border bg-card p-3 sm:p-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Switch checked={adminPreferences.twoFactorAuthentication} onCheckedChange={(checked) => updatePreference("twoFactorAuthentication", checked)} disabled={loading || saving}/>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-foreground">Force password reset every 90 days</span>
                <Switch checked={adminPreferences.forcePasswordReset} onCheckedChange={(checked) => updatePreference("forcePasswordReset", checked)} disabled={loading || saving}/>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-foreground">IP whitelist for admin access</span>
                <Switch checked={adminPreferences.ipWhitelist} onCheckedChange={(checked) => updatePreference("ipWhitelist", checked)} disabled={loading || saving}/>
              </div>
            </div>
          </motion.div>
