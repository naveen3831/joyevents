...
const AdminPayments = () => {
    ...
    const handleRefund = async (booking) => {
        const confirmed = confirm(`Are you sure you want to refund ${formatCurrency(booking.price)} for booking "${booking.serviceName || booking.eventName}"?\n\nThis will cancel the booking and mark the payment as refunded.\n\nThis action cannot be undone.`);
        if (!confirmed)
            return;
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden overflow-x-auto">
            ...
            {loading ? (<div className="flex items-center justify-center py-16 text-muted-foreground gap-2">
              ...
              </div>) : bookings.length === 0 ? (<div className="p-10 text-center text-muted-foreground">
                ...
                <table className="w-full text-sm min-w-[600px]">
                  ...
                  <tbody>
                    {bookings.map((booking) => (<tr key={booking._id} className="border-b border-border last:border-0 hover:bg-secondary/20 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            {booking.cardLast4 && (<span className="text-xs text-muted-foreground">****{booking.cardLast4}</span>)}
                            {booking.upiId && (<span className="text-xs text-muted-foreground">{booking.upiId}</span>)}
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`rounded-full px-3 py-1 text-xs font-semibold capitalize ${PAYMENT_STATUS_BADGE[booking.paymentStatus] || "bg-secondary text-muted-foreground"}`}>
