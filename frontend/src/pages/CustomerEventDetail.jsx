...
const CustomerEventDetail = () => {
    ...
    return (<CustomerLayout>
      <div className="min-h-screen">
        ...
        <div className="flex flex-col lg:flex-row min-h-[calc(100vh-4rem)] px-3 sm:px-6 lg:px-12 gap-4 sm:gap-8 pb-8 sm:pb-12 mt-4 sm:mt-6">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} className="lg:w-1/2 bg-card rounded-2xl border border-border overflow-hidden flex flex-col">
            ...
            <div className="relative aspect-[16/10] w-full overflow-hidden bg-secondary shrink-0">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8">
                {event.category && (<span className="inline-block rounded-full bg-primary/20 border border-primary/40 px-3 py-1 text-xs font-semibold text-primary mb-3">
                    {event.category}
                  </span>)}
                <h1 className="font-display text-4xl font-bold text-white leading-tight">{event.title}</h1>
                {/* Rating display */}
                {event.averageRating && event.averageRating > 0 ? (<div className="flex items-center gap-1.5 mt-2 text-white">
                ...
                {event.createdBy?.name && (<p className="mt-2 text-white/70 text-sm">
            ...
            { icon: Users, label: maxAttendees > 0 ? `${attendeesCount} / ${maxAttendees} attendees` : `${attendeesCount} attending` },
        ].map(({ icon: Icon, label }) => label && (<div key={label} className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              {/* Attendee progress */}
              {maxAttendees > 0 && (<div className="mb-6">
              ...
              {/* Ticket types info */}
              {event.eventType === "ticketed" && !event.hasMultipleSessions && event.tickets?.length > 0 && (<div>
                ...
                return (<div key={t.type} className="flex items-center justify-between rounded-lg border border-border bg-card px-4 py-2.5">
                          <div>
              ...
              {/* Gallery */}
              {event.gallery?.length > 0 && (<div className="mt-6">
                  ...
                  {showGallery && (<div className="grid grid-cols-3 gap-2 mt-2">
                      {event.gallery.map((img, idx) => (<button key={idx} onClick={() => setLightboxIndex(idx)} className="relative aspect-square rounded-lg overflow-hidden bg-secondary hover:opacity-90 transition-opacity">
              ...
              <div className="mt-8 border-t border-border pt-8">
                ...
                {reviews.length === 0 ? (<p className="text-sm text-muted-foreground italic">No reviews yet for this event.</p>) : (<div className="space-y-4">
                    {reviews.slice(0, 2).map((review) => (<div key={review._id} className="rounded-xl border border-border bg-secondary/20 p-4">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          {review.ratedAt && (<span className="text-[10px] text-muted-foreground">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="lg:w-1/2 bg-card rounded-2xl border border-border">
            <div className="p-8 max-w-lg mx-auto">
              ...
              {event.live ? (<div className="space-y-4">
                  ...
                  <div className="rounded-xl border border-border bg-secondary/30 p-4 space-y-3">
                    {event.price > 0 && (<div className="flex items-center justify-between">
                        <span className="text-sm text-muted-foreground">Price</span>
                        <span className="font-bold text-primary">{formatCurrency(event.price)}</span>
                      </div>)}
                    {event.datetime && (<div className="flex items-center justify-between">
                    ...
                    {event.location && (<div className="flex items-center justify-between">
                        <span className="text-sm text-muted-foreground">Location</span>
                        <span className="text-sm font-medium text-right max-w-[60%]">{event.location}</span>
                      </div>)}
                    {event.attendeesCount !== undefined && (<div className="flex items-center justify-between">
                ...
                </div>) : isSoldOut() ? (<div className="rounded-xl border border-red-500/30 bg-red-500/10 p-8 text-center">
                ...
                event.sessions?.day?.enabled && event.sessions?.night?.enabled && (<div>
                    ...
                    return (<button key={sess} disabled={soldOut} onClick={() => { setSelectedSession(sess); setSelectedTickets({}); }} className={`p-4 rounded-xl border-2 text-sm font-medium transition-all ${selectedSession === sess
                ...
                (!event.sessions?.day?.enabled && event.sessions?.night?.enabled)) && (<div className="space-y-2">
                ...
                !((event.sessions?.day?.enabled && !event.sessions?.night?.enabled) ||
                    (!event.sessions?.day?.enabled && event.sessions?.night?.enabled)) && (<div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-center text-sm text-amber-600 font-medium">
                ...
                const t = tickets.find((t) => t.type === type);
                return (<div key={type} className="flex justify-between text-sm text-muted-foreground mb-2">
                            <span className="capitalize">{type} × {qty}</span>
                            <span>{formatCurrency((t?.price || 0) * qty)}</span>
                          </div>);
            })}
                      {event.eventType === "fullService" && (<div className="flex justify-between text-sm text-muted-foreground mb-2">
                          <span>{fullServiceQty} × {formatCurrency(event.price)}</span>
                          <span>{formatCurrency(event.price * fullServiceQty)}</span>
                        </div>)}
                      {appliedPromo && (<div className="flex justify-between text-sm text-green-600 mb-2">
                          <span>Discount</span>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <Button variant="outline" className="flex-1 h-12 text-base border-primary/40 hover:bg-primary/5 text-primary" onClick={() => handleAddToCart(false)} disabled={(event.eventType === "ticketed" && Object.values(selectedTickets).reduce((s, q) => s + q, 0) === 0) ||
                (event.eventType === "fullService" && fullServiceQty === 0)}>
                      <ShoppingBag className="mr-2 h-5 w-5"/>
                      Add to Cart
                    </Button>
                    <Button className="flex-1 h-12 text-base bg-gradient-primary text-primary-foreground hover:opacity-90" onClick={() => handleAddToCart(true)} disabled={(event.eventType === "ticketed" && Object.values(selectedTickets).reduce((s, q) => s + q, 0) === 0) ||
                (event.eventType === "fullService" && fullServiceQty === 0)}>
      ...
      {/* Gallery Lightbox */}
      {lightboxIndex !== null && event?.gallery?.length > 0 && (<div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4" onClick={() => setLightboxIndex(null)}>
          <button className="absolute top-4 right-4 text-white/70 hover:text-white" onClick={() => setLightboxIndex(null)}>
            <X className="h-7 w-7"/>
          </button>
          <button className="absolute left-4 top-1/2 -translate-y-1/2 text-white/70 hover:text-white text-3xl font-bold px-3" onClick={e => { e.stopPropagation(); setLightboxIndex(i => (i - 1 + event.gallery.length) % event.gallery.length); }}>‹</button>
          <img src={imgSrc(event.gallery[lightboxIndex])} alt={`Gallery ${lightboxIndex + 1}`} className="max-h-[85vh] max-w-full object-contain rounded-lg shadow-2xl" onClick={e => e.stopPropagation()}/>
          <button className="absolute right-4 top-1/2 -translate-y-1/2 text-white/70 hover:text-white text-3xl font-bold px-3" onClick={e => { e.stopPropagation(); setLightboxIndex(i => (i + 1) % event.gallery.length); }}>›</button>
