...
const CustomerServiceDetail = () => {
    ...
    const getFinalPrice = () => {
        ...
        const addOnTotal = (service.addOns || [])
            .reduce((sum, a) => sum + (Number(a.price) * (selectedAddOns[a.name] || 0)), 0);
        ...
        addToCart({
            ...
            details: {
                addOns: (service.addOns || [])
                    ...
                    .map((a) => ({ name: a.name, price: a.price, quantity: selectedAddOns[a.name] || 1 })),
    ...
    return (<CustomerLayout>
        ...
        <div className="flex flex-col lg:flex-row min-h-[calc(100vh-4rem)] px-3 sm:px-6 lg:px-12 gap-4 sm:gap-8 pb-8 sm:pb-12 mt-4 sm:mt-6">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} className="lg:w-1/2 bg-card rounded-2xl border border-border overflow-hidden flex flex-col">
            ...
            <div className="border-t border-border p-8 flex-1">
              ...
              {service.addOns?.length > 0 && (<div className="mb-6">
                  <h3 className="font-semibold text-base mb-3 flex items-center gap-2">
              ...
              {service.createdBy && (<div className="rounded-lg bg-card border border-border p-4">
              ...
              {/* Gallery */}
              {service.gallery?.length > 0 && (<div className="mt-6">
            ...
            <div className="p-8 max-w-lg mx-auto">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="date" min={new Date().toISOString().split("T")[0]} value={date} onChange={e => setDate(e.target.value)} className="w-full rounded-lg border border-border bg-secondary px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary"/>
                    </div>
                    <div>
                      <label className="block text-xs text-muted-foreground mb-1">Select Time</label>
                      <input type="time" value={time} onChange={e => setTime(e.target.value)} className="w-full rounded-lg border border-border bg-secondary px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary"/>
                    </div>
                  </div>
                </div>

                {/* Add-ons */}
                {service.addOns?.length > 0 && (<div className="border-t border-border pt-6">
                ...
                if (!addon.showGuestCount) {
                    return (<div key={addon.name} onClick={() => setSelectedAddOns(p => { const n = { ...p }; if (n[addon.name])
                        delete n[addon.name];
                    else
                        n[addon.name] = 1; return n; })} className={`flex items-center justify-between gap-3 rounded-xl border-2 px-4 py-3 cursor-pointer transition-all ${qty > 0 ? "border-primary bg-primary/10" : "border-border bg-card hover:bg-secondary"}`}>
                ...
                return (<div key={addon.name} className={`flex items-center justify-between gap-3 rounded-xl border-2 px-4 py-3 transition-all ${qty > 0 ? "border-primary bg-primary/10" : "border-border bg-card"}`}>
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium">{addon.name}</p>
                              <p className="text-xs text-primary font-semibold">{formatCurrency(addon.price)} per {label}</p>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                              <button disabled={qty <= 0} onClick={() => setSelectedAddOns(p => { const n = { ...p }; if ((n[addon.name] || 0) > 1)
                    n[addon.name]--;
                else
                    delete n[addon.name]; return n; })} className="w-7 h-7 rounded-lg bg-secondary hover:bg-secondary/80 font-bold disabled:opacity-40 transition-colors text-sm">−</button>
                              <div className="text-center w-16">
                                <span className={`font-bold text-sm ${qty > 0 ? "text-primary" : "text-muted-foreground"}`}>{qty || 0}</span>
                                <p className="text-[10px] text-muted-foreground leading-none">{label}</p>
                              </div>
                              <button onClick={() => setSelectedAddOns(p => ({ ...p, [addon.name]: (p[addon.name] || 0) + 1 }))} className="w-7 h-7 rounded-lg bg-secondary hover:bg-secondary/80 font-bold transition-colors text-sm">+</button>
                            </div>
                          </div>);
            })}
                    </div>
                ...
                <div className="border-t border-border pt-6">
                  <div className="rounded-xl bg-primary/10 border border-primary/20 p-5">
                    ...
                    {Object.keys(selectedAddOns).length > 0 && (<div className="space-y-1 mb-2">
                        {(service.addOns || []).filter((a) => (selectedAddOns[a.name] || 0) > 0).map((a) => (<div key={a.name} className="flex items-center justify-between">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <Button variant="outline" className="flex-1 h-12 text-base border-primary/40 hover:bg-primary/5 text-primary" onClick={() => handleAddToCart(false)}>
                    <ShoppingBag className="mr-2 h-5 w-5"/>
                    Add to Cart
                  </Button>
                  <Button className="flex-1 h-12 text-base bg-gradient-primary text-primary-foreground hover:opacity-90" onClick={() => handleAddToCart(true)}>
                    Request Now
                  </Button>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 sm:gap-8 lg:grid-cols-4">
              {relatedServices.map((r) => (<div key={r._id} onClick={() => navigate(`/customer-dashboard/services/${r._id}`)} className="group rounded-2xl border border-border bg-card overflow-hidden flex flex-col hover:border-primary/50 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 cursor-pointer">
                  ...
                  <div className="p-3 sm:p-5 flex flex-col flex-1">
                    <Link to={`/customer-dashboard/services/${r._id}`}>
      ...
      {/* Gallery Lightbox */}
      {lightboxIndex !== null && service?.gallery?.length > 0 && (<div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4" onClick={() => setLightboxIndex(null)}>
          <button className="absolute top-4 right-4 text-white/70 hover:text-white" onClick={() => setLightboxIndex(null)}>
            <X className="h-7 w-7"/>
          </button>
          <button className="absolute left-4 top-1/2 -translate-y-1/2 text-white/70 hover:text-white text-3xl font-bold px-3" onClick={e => { e.stopPropagation(); setLightboxIndex(i => (i - 1 + service.gallery.length) % service.gallery.length); }}>‹</button>
          ...
          <button className="absolute right-4 top-1/2 -translate-y-1/2 text-white/70 hover:text-white text-3xl font-bold px-3" onClick={e => { e.stopPropagation(); setLightboxIndex(i => (i + 1) % service.gallery.length); }}>›</button>
