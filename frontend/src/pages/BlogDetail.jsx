...
import { BLOG_POSTS, imgSrc } from "@/data/blogPosts";
const BlogDetail = () => {
    const { id } = useParams();
    const postId = Number(id);
    const post = useMemo(() => BLOG_POSTS.find((item) => item.id === postId), [postId]);
    if (!post) {
        return (<Layout>
        ...
        <div className="grid gap-10 lg:grid-cols-[2fr_1fr]">
          <article className="space-y-8">
            <div className="overflow-hidden rounded-3xl border border-border bg-card shadow-sm">
              <img src={imgSrc(post.image)} alt={post.title} className="h-80 w-full object-cover"/>
            </div>
          ...
          <aside className="space-y-6">
            ...
            <div className="rounded-3xl border border-border bg-card p-6">
              ...
              <div className="mt-4 space-y-3">
                {BLOG_POSTS.filter((item) => item.id !== post.id).slice(0, 3).map((item) => (<Link key={item.id} to={`/blog/${item.id}`} className="block rounded-2xl border border-border bg-secondary/70 p-3 transition-colors hover:border-primary/50 hover:bg-primary/10">
